...
extern void hell_watch_print(char *msg);
...
//static uint8_t sd_raw_rec_byte(void); // no-longer private, used by diskio.c
//static uint8_t sd_raw_send_command(uint8_t command, uint32_t arg); // no-longer private, used by diskio.c
...
uint8_t sd_raw_init()
{
	...
	//FIXME: use uart as spi master to speed up
#else
...
void sd_raw_send_byte(uint8_t b)
{
	...
	for (uint8_t i = 0; i < 8; i++) {
	  CLR_SCK();//fastDigitalWrite(SPI_SCK_PIN, LOW);
	  ...
	  b <<= 1;
	  SET_SCK();//fastDigitalWrite(SPI_SCK_PIN, HIGH);
	}
...
uint8_t sd_raw_rec_byte()
{
	...
	SET_MOSI();//fastDigitalWrite(SPI_MOSI_PIN, HIGH);
    ...
    for(uint8_t i = 0; i < 18; ++i)
    {
        ...
        else
        {
...
#endif
            {
                ...
                if (csd_read_bl_len == 9)
                {
                    info->block_size = 512;
                }
                else if (csd_read_bl_len == 10)
                {
                    info->block_size = 1024;
                }
                else if (csd_read_bl_len == 11)
                {

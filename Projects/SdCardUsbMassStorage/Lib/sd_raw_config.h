...
#elif defined(HELL_WATCH_PORT)
    #define SD_MOSI			1
    #define SD_SCK			2
    #define SD_SS			3
    #define SD_MISO			0

    #define PE_PIN_LOW(pin)		(PORTE.OUTCLR = _BV(pin))
    #define PE_PIN_HIGH(pin)		(PORTE.OUTSET = _BV(pin))
    #define PE_PIN_TOGGLE(pin)	(PORTE.OUTTGL = _BV(pin))
				
    #define PE_PIN_INPUT(pin)		(PORTE.DIRCLR = _BV(pin))
    #define PE_PIN_OUTPUT(pin)	(PORTE.DIRSET = _BV(pin))

	#define GET_MISO()				(PORTE.IN & _BV(SD_MISO))

    #define SET_MOSI()				PE_PIN_HIGH(SD_MOSI)
    #define CLR_MOSI()				PE_PIN_LOW(SD_MOSI)

    #define SET_SS()				PE_PIN_HIGH(SD_SS)
    #define CLR_SS()				PE_PIN_LOW(SD_SS)

    #define SET_SCK()				PE_PIN_HIGH(SD_SCK)
    #define CLR_SCK()				PE_PIN_LOW(SD_SCK)
    #define TOGGLE_SCK()			PE_PIN_TOGGLE(SD_SCK)

    #define configure_pin_mosi()	PE_PIN_OUTPUT(SD_MOSI)
    #define configure_pin_sck()	PE_PIN_OUTPUT(SD_SCK)
    #define configure_pin_ss() 	PE_PIN_OUTPUT(SD_SS)
    #define configure_pin_miso()	PE_PIN_INPUT(SD_MISO)

    #define select_card() 			CLR_SS()
    #define unselect_card() 		SET_SS()
#else
